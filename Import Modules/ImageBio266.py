...
def run(logfile):
    ...
    try:
        ...
        iolite_dataframe.columns = ['Timestamp', 'Sequence Number', 'SubPoint Number', 'Vertex Number',
                                    ...
                                    'X(um)', 'Y(um)', 'Intended X(um)', 'Intended Y(um)',
                                    'Scan Velocity (um/s)',
                                    'Laser State', 'Laser Rep. Rate (Hz)', 'Spot Type', 'Spot Size (um)', 'A', 'B', 'C',
                                    'D', 'E']
    ...
    if area_ablation is True:
        ...
        scan_speed_array = iolite_dataframe[
            (iolite_dataframe['Laser State'] == 'On') & (iolite_dataframe['Scan Velocity (um/s)'].notna())][
            'Scan Velocity (um/s)'].to_numpy()
